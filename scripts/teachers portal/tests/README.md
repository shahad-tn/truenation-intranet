...
| `events.test.js` | Step 2b: a fake CalendarApp. Each calendar gets exactly the sessions the rules give (readers skip Q&A and Feed The Sheep, graphics skip Q&A), times and durations are right (incl. a Sheets time value), the public event has no link, preview writes nothing, a second run creates nothing, a run stopped at its time limit or killed mid-way finishes on re-run with no duplicates, a row deleted mid-run does not shift ids onto the wrong sessions, and calendar and sheet calls stay within budget |
